...
export default function MensajesAlumno() {
  ...
  const [adminNombre, setAdminNombre] = useState('Administración');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => { if (data.user) setUserId(data.user.id); });
    // Obtener admin via server action (bypassa RLS)
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('mensajes-alumno')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'mensajes_internos' }, (payload) => {
        ...
        // Si es un mensaje individual que me involucra
  ...
  const enviarMsg = async () => {
    ...
    if (res.success) { setNuevoMsg(''); /* Realtime lo agrega automáticamente */ }
        ...
        <TabsContent value="chat">
          <Card className="rounded-2xl overflow-hidden border">
            <div className="p-3 bg-white border-b flex items-center gap-3">
              ...
              <div><p className="font-bold text-sm">Administración {inst.nombre_corto}</p><p className="text-[10px] text-muted-foreground">Soporte y Atención Estudiantil</p></div>
            </div>
            <div className="h-[50vh] overflow-y-auto p-4 space-y-2 bg-[#f0ebe3]">
              {mensajesChat.length === 0 && <div className="text-center text-muted-foreground text-sm py-10"><MessageSquare size={32} className="mx-auto mb-2 opacity-20" /><p>Inicia una conversación con Administración</p></div>}
              {mensajesChat.map(m => {
