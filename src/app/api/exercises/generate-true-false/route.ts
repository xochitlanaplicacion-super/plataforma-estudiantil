...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = `Eres un experto creador de exámenes educativos. Basado en el texto o instrucción provista, debes extraer/generar exactamente ${numPreguntas} enunciados declarativos precisos.
Para cada enunciado, debes determinar si es verdadero o falso, asegurando que no haya ambigüedad.

Devuelve ÚNICAMENTE la respuesta en formato JSON con la siguiente estructura exacta:
{
  "items": [
    {
      "statement": "El agua hierve a 100 grados Celsius a nivel del mar.",
      "correct": true,
      "feedback": "El punto de ebullición del agua a presión atmosférica normal es de 100°C."
    },
    {
      "statement": "El sol gira alrededor de la tierra.",
      "correct": false,
      "feedback": "La Tierra gira alrededor del Sol debido a la gravedad (heliocentrismo)."
    }
  ]
}

REGLAS ESTRICTAS:
- Genera exactamente ${numPreguntas} enunciados.
- El JSON debe tener la llave "items".
- El campo "correct" debe ser estrictamente un booleano (true o false).
- En el campo "feedback", escribe una breve justificación de por qué la afirmación es verdadera o falsa.
- No incluyas explicaciones adicionales fuera del JSON, ni bloques markdown, solo el JSON puro.`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Error al generar verdadero/falso con IA." }, { status: 500 });
    }
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      supabaseAdmin.from('ai_token_usage').insert({
        user_id: userId || null,
        tipo_peticion: 'generar_verdadero_falso',
        clase_tema: 'VERDADERO_FALSO_IA',
    ...
    console.error("Error en generación AI de verdadero/falso:", error);
