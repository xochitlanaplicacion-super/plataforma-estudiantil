...
export async function loadTeacherParticipationAction(assignmentIdInput?: unknown): Promise<{ ok: true; data: TeacherParticipationData } | { ok: false; message: string }> {
  try {
    ...
    const summaryResult = await session.supabase.rpc('obtener_resumen_participacion_docente_movil_unificado', { p_asignacion_id: selectedId });
    ...
    return {
      ...
      data: {
        ...
        summary: summarySchema.parse(summaryResult.data),
      },
    };
  } catch (error) {
    ...
    return {
      ok: false,
      message: 'No se pudo cargar la participación. Actualiza la página e inténtalo nuevamente.',
    };
  }
}
