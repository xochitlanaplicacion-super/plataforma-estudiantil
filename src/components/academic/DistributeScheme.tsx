...
export function DistributeScheme({ data, scheme, onApplied }: { data: AcademicConfigurationDto; scheme: AcademicSchemeConfigurationDto; onApplied?: () => Promise<void> }) {
  ...
  return <section className="mt-4 space-y-3 rounded-xl border p-4">
    ...
    {enabled && <>
      <p className="text-sm">Se guardarán copias independientes para el mismo periodo: {data.periods.find((row) => row.id === scheme.periodId)?.name}. También se copiarán los ajustes de captura móvil: mínima, incremento, lector QR y confirmación. No se copian calificaciones.</p>
      ...
      <div className="max-h-64 space-y-2 overflow-auto">{options.map((row) => <label key={row.id} className="flex items-center gap-3 rounded-lg border p-3"><input type="checkbox" checked={selected.includes(row.id)} disabled={busy} onChange={(event) => setSelected((ids) => event.target.checked ? [...ids, row.id] : ids.filter((id) => id !== row.id))} />{label(row.id)}{data.schemes.some((item) => item.assignmentId === row.id && item.periodId === scheme.periodId) ? ' · Ya tiene criterios (se conservarán y se actualizarán los ajustes de la app)' : ''}</label>)}</div>
