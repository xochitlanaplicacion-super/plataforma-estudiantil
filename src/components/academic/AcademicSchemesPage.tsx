  ...
  return (
    <li className="space-y-4 rounded-lg border p-4">
      <div className="grid gap-3 md:grid-cols-[minmax(0,2fr)_minmax(8rem,1fr)_7rem_5rem_auto]">
        ...
        <div className="space-y-1"><Label htmlFor={`criterion-type-${criterion.id}`}>Tipo</Label><select id={`criterion-type-${criterion.id}`} className={fieldClassName} disabled={disabled} value={draft.type} onChange={(event) => setDraft((value) => ({ ...value, type: event.target.value as typeof value.type }))}><option value="directo">Directo</option><option value="actividades">Actividades</option><option value="participacion">Participación</option><option value="hibrido">Híbrido</option></select></div>
        ...
        <>
          ...
          {data.assignments.length === 0 || data.periods.length === 0 ? (
            ...
            <>
              ...
              <Card>
                <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Scale aria-hidden="true" />Reglas del esquema</CardTitle><CardDescription>Aprobatoria, visualización y versión aplicable al alcance.</CardDescription></div><Button variant="outline" disabled={!assignmentId || !periodId} onClick={() => chooseScheme('')}><Plus />Nuevo borrador</Button></div></CardHeader>
                <CardContent className="space-y-5">
                  ...
                  <form className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(event) => { event.preventDefault(); setConfirmation('scheme'); }}>
                ...
                <div className={teacherView ? 'grid gap-6' : 'grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(20rem,0.6fr)]'}>
                  <Card>
                    ...
                    <CardContent className="space-y-5">
                      ...
                      {!readOnly ? <fieldset className="grid gap-3 rounded-lg border border-dashed p-4 sm:grid-cols-[minmax(0,2fr)_minmax(8rem,1fr)_7rem_5rem_auto]"><legend className="px-1 font-medium">Nuevo criterio</legend><Input aria-label="Nombre del nuevo criterio" placeholder="Ej. Examen" value={newCriterion.name} onChange={(event) => setNewCriterion((value) => ({ ...value, name: event.target.value }))} /><select aria-label="Tipo del nuevo criterio" className={fieldClassName} value={newCriterion.type} onChange={(event) => setNewCriterion((value) => ({ ...value, type: event.target.value as typeof value.type }))}><option value="directo">Directo</option><option value="actividades">Actividades</option><option value="participacion">Participación</option><option value="hibrido">Híbrido</option></select><Input aria-label="Peso del nuevo criterio" type="number" min={0} max={100} step="0.0001" value={newCriterion.weight} onChange={(event) => setNewCriterion((value) => ({ ...value, weight: Number(event.target.value) }))} /><Input aria-label="Orden del nuevo criterio" type="number" min={1} max={99} value={newCriterion.order} onChange={(event) => setNewCriterion((value) => ({ ...value, order: Number(event.target.value) }))} /><Button type="button" disabled={!newCriterion.name} onClick={() => void addCriterion()}><Plus />Agregar</Button></fieldset> : null}
                      <div className="flex flex-wrap gap-2"><Button type="button" disabled={!activationReady} onClick={() => setConfirmation('activate')}><CheckCircle2 />Activar esquema</Button>{selectedScheme.state !== 'borrador' ? <><Input aria-label="Nombre de la nueva versión" className="max-w-sm" value={copyName} onChange={(event) => setCopyName(event.target.value)} /><Button type="button" variant="outline" disabled={!copyName.trim()} onClick={() => setConfirmation('copy')}><Copy />Crear copia editable</Button></> : null}</div>
