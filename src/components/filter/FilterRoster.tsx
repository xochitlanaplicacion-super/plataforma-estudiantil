...
import { ContactRound, Download, FileSpreadsheet, Plus, Search, Trash2, Upload, Users } from 'lucide-react';
import { addFilterGuardianContact, addFilterStudents, deleteFilterStudents, upsertFilterStructure } from '@/lib/actions/filter-control';
...
export function FilterRoster({ initialData }: { initialData: any }) {
  ...
  const [contactStudentId, setContactStudentId] = useState(initialData.students[0]?.id || ''); const [contactName, setContactName] = useState('');
  const [contactRelationship, setContactRelationship] = useState('madre'); const [contactPhone, setContactPhone] = useState(''); const [contactEmail, setContactEmail] = useState('');
  ...
  return <div className="space-y-6">
    ...
    {initialData.canManageGuardians && <Card><CardHeader><CardTitle className="flex items-center gap-2"><ContactRound className="h-5 w-5 text-primary" />Contactos oficiales para entregas</CardTitle><CardDescription>Sólo dirección y administración pueden verificar estos contactos. Después podrán reutilizarse en el futuro registro QR de familias.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div><Label>Alumno *</Label><Select value={contactStudentId} onValueChange={setContactStudentId}><SelectTrigger><SelectValue placeholder="Selecciona alumno" /></SelectTrigger><SelectContent>{initialData.students.map((student: any) => <SelectItem key={student.id} value={student.id}>{student.full_name}</SelectItem>)}</SelectContent></Select></div><div><Label>Nombre del padre, madre o tutor *</Label><Input value={contactName} onChange={(e) => setContactName(e.target.value)} /></div><div><Label>Parentesco *</Label><Select value={contactRelationship} onValueChange={setContactRelationship}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="madre">Madre</SelectItem><SelectItem value="padre">Padre</SelectItem><SelectItem value="tutor">Tutor(a)</SelectItem></SelectContent></Select></div><div><Label>Teléfono oficial</Label><Input value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} inputMode="tel" /></div><div><Label>Correo oficial</Label><Input value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} type="email" /></div></div><Button disabled={pending || !contactStudentId || contactName.trim().length < 2 || (!contactPhone.trim() && !contactEmail.trim())} onClick={() => run(() => addFilterGuardianContact({ studentId: contactStudentId, fullName: contactName, relationship: contactRelationship, phone: contactPhone, email: contactEmail }), 'Contacto oficial verificado', () => { setContactName(''); setContactPhone(''); setContactEmail(''); })}><Plus className="mr-2 h-4 w-4" />Guardar contacto oficial</Button>{contactStudentId && <div className="rounded-lg border p-3"><p className="text-sm font-semibold">Contactos activos del alumno</p><div className="mt-2 space-y-2">{initialData.contacts.filter((item: any) => item.student_id === contactStudentId).map((item: any) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-muted/40 px-3 py-2 text-sm"><span>{item.full_name} · {item.relationship}</span><span className="text-xs text-muted-foreground">{item.phone || item.email} · {item.verification_status}</span></div>)}{!initialData.contacts.some((item: any) => item.student_id === contactStudentId) && <p className="text-sm text-muted-foreground">Sin contactos oficiales registrados.</p>}</div></div>}</CardContent></Card>}
  </div>;
}
