...
export function FilterEarlyDepartureWizard({ initialData, initialClock }: { initialData: any; initialClock: any }) {
  ...
  const makeDraft = useCallback((status = draftStatus): EarlyDepartureDraft => ({
    ...
    identificationEvidence: persistFile(identificationEvidence), pickupPersonPhoto: persistFile(pickupPersonPhoto), finalHandoverPhoto: persistFile(finalHandoverPhoto),
  }), [draftStatus, finalHandoverPhoto, identificationEvidence, pickupPersonPhoto, requestId, step, student, values]);
  ...
  useEffect(() => {
    getEarlyDepartureDraft(draftScope).then((draft) => {
      if (draft?.version === 1 && draft.clientRequestId) {
        ...
        setIdentificationEvidence(restoreFile(draft.identificationEvidence)); setPickupPersonPhoto(restoreFile(draft.pickupPersonPhoto)); setFinalHandoverPhoto(restoreFile(draft.finalHandoverPhoto));
  ...
  const formDataFromDraft = (draft: EarlyDepartureDraft) => {
    ...
    const identification = restoreFile(draft.identificationEvidence); const pickup = restoreFile(draft.pickupPersonPhoto); const handover = restoreFile(draft.finalHandoverPhoto);
    if (identification) data.set('identificationEvidence', identification); if (pickup) data.set('pickupPersonPhoto', pickup); if (handover) data.set('finalHandoverPhoto', handover);
    return data;
  };
  const resetForm = useCallback(async () => {
    ...
    setIdentificationEvidence(null); setPickupPersonPhoto(null); setFinalHandoverPhoto(null); setResetProgress(0); setResetOpen(false);
  ...
  const sendDraft = useCallback(async (draft: EarlyDepartureDraft, automatic = false) => {
    ...
    try {
      ...
      toast({ title: result.duplicate ? 'La salida ya estaba registrada' : 'Salida anticipada guardada y auditada', description: 'Supabase confirmó el registro; el borrador local ya puede retirarse.' });
